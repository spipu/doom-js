...
- **Difficulty selection**: after picking a WAD, a difficulty screen offers the five vanilla Doom skills on a generic scale (*Very easy* … *Very hard*, the skill number on the second line) plus a skill 0, **No monsters** — the skill-1 world with every monster removed. The chosen skill drives the single-player thing filtering below and is kept across the level chain; the per-skill gameplay rules (spawn filter bit, ammo factor — Doom ×2, Heretic ×1.5 on the easiest/hardest skills — and the half player damage on the easy skills, applied before armor like vanilla) are game-profile data transcribed from the UZDoom MAPINFO skill blocks.
