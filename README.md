...
- **Automap**: a translucent plan of the level over the running game — you keep playing under it, the virtual pad stays on top. Fixed framing on the whole level, north up, the player as a green arrow, walls and floor/ceiling steps in the colour code of the loaded game, and every locked door stroked thicker in the colour of the key it wants. It reveals itself like the original — a wall is remembered only when it is really seen, so a room behind a closed door stays dark until the door opens — with one rule of ours: nothing is memorised beyond 20 m. A map power-up greys in everything still unseen, which is what those two items now do. Absent on a WAD whose BSP is unusable.
