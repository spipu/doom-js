...
- **Monsters** (killable phase): every monster of the loaded game spawns from the THINGS lump — the full Doom/Doom 2 bestiary (zombies to the Cyberdemon, Spider Mastermind, Commander Keen and the MAP30 Boss Brain) and the Heretic one (gargoyles to the Maulotaur and D'Sparil's serpent), 35 definitions transcribed from the UZDoom zscript actors and owned by the game profiles. Each monster is a billboard with the **eight vanilla rotation views** (mirrored lumps like `TROOA2A8` are flipped at decode time, anchors mirrored with them), faces its THINGS angle, animates at 35 Hz and blocks the player with its Doom hitbox; the Spectre and the Heretic ghosts render translucent. Monsters **take damage from every player attack channel** — hitscan pellets, projectile direct hits, explosion splash and the BFG's shooter-side 40-ray spray — through a faithful `P_DamageMobj` pipeline: blood splashes (Doom's damage-staged `BLUD`, Heretic's `BLOD`), pain flinches, death and over-kill **gib** animations (`health < -spawnhealth × gibfactor`, Doom 1 / Heretic 0.5), and the vanilla **kickback thrust** (`damage × 0.125 × kickback / mass`, friction 0.90625/tic) that shoves monsters *and corpses* around — bodies slide against walls, climb steps up to 24 units, refuse moves into the void and fall off ledges; a corpse standing on a lift keeps riding it. A dying monster drops its vanilla items (clip, shotgun, chaingun, Heretic ammo — dropped weapon ammo halved) as pickable billboards that inherit a moving floor. **Doom barrels and Heretic pods** are shootable bodies with vanilla chain explosions (radius damage with true line-of-sight — walls *and* floor slabs block a blast — Chebyshev falloff, blast thrust): one pistol shot in MAP23 sets off the whole yard. Kills are tallied on the HUD (`☠ x/y`; barrels, pods and the Boss Brain never count) and hitscan shots stop at the first body like `PTR_ShootTraverse` (a bullet never reaches a gun-trigger line behind its victim). Hunting AI and monster attacks come with the next phases of the chantier.
...
│   │   ├── monster/            Monster system (killable phase)
...
│   │   │   ├── doomMonsterSystem.js   35 Hz runtime driver: state machine + actions, velocity integration (knockback, gravity), rotation views, ray↔body cylinder tests
│   │   │   └── doomMonsterDamage.js   Shared P_DamageMobj pipeline: blood, health, thrust, death/gib, pain — and P_RadiusAttack (LOS, Chebyshev falloff)
│   │   ├── hud/
