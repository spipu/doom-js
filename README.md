...
- **On-the-fly conversion** (`js/doom/wad/convert/`): geometry, textures, animations and movers are instantiated directly into in-memory engine objects — no generated file, no server. Two-sided middle textures show only from the side that carries them, and the holes of the original maps (missing upper or lower textures) are filled with the neighbouring flat, as the hardware ports do.
