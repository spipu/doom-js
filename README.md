...
- **Save / load**: five save slots per WAD (deleted with the WAD). A save captures the full game state — player, movers mid-travel, consumed triggers, monsters, counters — and loading rebuilds the level and restores it exactly, missiles in flight included; transient visuals (decals, sprite effects…) are not kept, and saving is refused while dead.
...
- **Player equipment & pickups**: weapons / shared ammo pool / keys / timed effects / armor, with the vanilla pickup rules (a full item stays on the ground, weapons hand out ammo, armor classes); weapons, ammo and armor persist across levels, keys and effects reset. An item is reached inside a cylinder — a ground circle plus a vertical window — so one left on a ledge is taken from the floor below, without jumping. Locked doors require their key. Every power-up with a live effect shows a labelled line above the health bar (timed ones with a countdown, berserk permanently) and its own screen feedback: golden wash for invulnerability, green tint for the radiation suit, scene-wide night vision for the light visor and Heretic torch, translucent weapon for partial invisibility — each blinking through the vanilla end-of-effect warning.
...
- **Automap**: a translucent plan of the level over the running game — you keep playing under it, the virtual pad stays on top. Fixed framing on the whole level — north up, or turned a quarter when the level is taller than wide, to fill the panel — the player as a green arrow, walls and floor/ceiling steps in the colour code of the loaded game, and every locked door stroked thicker in the colour of the key it wants. It reveals itself like the original — a wall is remembered only when it is really seen, so a room behind a closed door stays dark until the door opens — with one rule of ours: nothing is memorised beyond 20 m. A map power-up greys in everything still unseen, which is what those two items now do. Absent on a WAD whose BSP is unusable.
