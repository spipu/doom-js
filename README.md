...
- **Help modal & persistent settings**: a `?` button on the WAD screen (also reached by pressing Up from the first list item) opens a stacked help modal — breadcrumb title, same mouse/keyboard/gamepad navigation as the screens — with a **Display** page (the `display.*` options — the interface language, the aiming crosshair, the distance light diminishing and the texture smoothing toggles), a **Controls** page that adapts to the active input device (named gamepad / virtual gamepad / keyboard+mouse) and an **About** page. The settings lists are auto-built from a definitions table and persisted in IndexedDB, one row per setting in one of three shapes: a **toggle** (Oui/Non — per-device vertical-look inversion, the display options), a **value list** of any length whose entries carry a code and a label, stepped through by activating the row (the language, `fr`/`en` — stored and editable, not consumed yet since no text is localized), and a **key binding** for the **full keyboard remapping** (one physical key per action — activate a binding, press the new key; a key already bound elsewhere is unbound from its old action, which becomes inert until remapped). A reset entry wipes every saved setting after confirmation.
