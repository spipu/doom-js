...
- **WAD menu**: selecting a WAD opens its own menu — *New game*, *Load game*, *Options*, *About* and *Quit &lt;WAD&gt;* — titled with the WAD name stripped of its `.wad` extension (like every sub-screen subtitle).
- **Save / load**: five save slots per WAD, stored in IndexedDB next to the WAD itself (deleting a WAD deletes its saves in the same transaction). The slots modal always shows the five slots — a used one displays its level, difficulty and locale-formatted date/time plus a delete cross (confirmed, like the WAD list), a free one reads *Empty* (selectable to save, inert to load) — with a bottom *Back* button in both modes; saving over a used slot asks for confirmation, and the refreshed list is the save feedback. Loading — from the WAD menu or from the pause menu, which replaces the running game — rebuilds the level deterministically (same WAD + level + skill → same instance codes) and patches the saved state on top: full player equipment (keys and timed effects included), position/orientation, movers mid-travel (doors, lifts, crushers, keyframe variants, riding bodies), consumed triggers (W1 zones, G1 lines, switch states with their SW1/SW2 faces), picked-up items, "+change" floor mutations, secrets/kills counters, every monster (position, state frame, health, target, corpses with their unblocking, nightmare respawns, dropped items) and the P_Random index. Deliberately not saved (transient/cosmetic): in-flight projectiles, sprite effects, decals, sector light phases and sound targets — and saving is refused while dead (an information modal says so).
...
- **Pause menu**: `ESC` (gamepad button 9, `≡` on the virtual pad) freezes the game — nothing moves, animated textures and screen flashes included — under a translucent overlay showing "{WAD} — Episode {n}" and five entries (*Resume*, *Load game* and *Save game* — the save slots modal, stacked, capturing the frozen game — then *Options* — the same modal as the WAD menu's, stacked — and *Leave the level*, back to the WAD menu). `ESC` again, Backspace or the gamepad back button resume; a click/Enter resume re-grabs the mouse (an `ESC` resume cannot — the browser only grants the pointer lock on a real user activation — click the canvas to re-aim). The game over screen returns to the WAD menu too, and the skill of the interrupted game stays preselected.
...
- **Moving elements & triggers**: doors (local, remote/tagged, fast, key-locked), lifts (switch/walk, fast, lower-to-lowest/highest/+8), rising floors (fixed +24/+32 and computed targets: lowest surrounding ceiling incl. crush variants, next-higher floor, shortest lower texture around the sector for `raiseToTexture` 30/96 — with a deliberate 1 s static pre-frame so the player can step onto the platform), **build-stairs** (specials 7/8/100/127), and teleporters (linedef → thing-14 landing). All behaviour tables (speeds, targets, W1/WR/S1/SR modes, open-stay vs open-wait-close) are aligned with the original linuxdoom source (`p_spec.c`/`p_switch.c`/`p_doors.c`/`p_floor.c`/`p_plats.c`). A generic **trigger → target instances** model drives them: a **switch** (press), a **walk-over zone** (an invisible instance at the linedef that fires when the player's move actually **crosses** the line, vanilla `P_CrossSpecialLine` — walking alongside it, or standing on it, does nothing — whatever his height, e.g. atop a raised lift), or a teleport pad (same crossing rule). Vanilla collects the crossed lines over the player's *box*, so brushing past the end of a line still fires it: the crossing keeps that player-radius tolerance. A displacement too large to be a walk step (teleport arrival, restored save) is never read as a crossing. A **tagged** door is driven remotely: crossing one of its trigger lines opens every door of that tag, so e.g. grabbing a key ringed by trigger lines opens the doors elsewhere (a local, tag-0 door still opens by walking into it). A door that also carries a manual line keeps its press whatever the remote lines say — vanilla puts the activation on each linedef and the sector is only the target, so a walk line never disables the DR of its own door. **Switches** sit on a one-sided middle wall *or* on the lower (step riser) / upper of a two-sided line, from either side, and their `SW1`↔`SW2` graphic swaps both ways; a switch line with no `SW` graphic (e.g. an SR lift edge) still works, as an invisible USE zone that fires its targets. A `SW` graphic painted on a **moving** face — a door panel, a lift / rising-floor / stair riser — is left to that mover's own mesh (a static panel over it would z-fight at rest and hang in the air once it moves): the line becomes an invisible USE zone whose `SW1`↔`SW2` swap is applied to the mover's faces. Lift risers are self-contained (every perimeter edge, neighbour-then-fallback texture). A **two-way floor elevator** — one line per side, a raise on one wall and a lower on the other (E1M8's shaft to the health ledge) — works both ways: a lower line aimed at a raised floor walks it back down, the mirror of a raise line aimed at a lowered lift. **Stairs** build a chain of one-way rising steps from the tagged base sector through adjacent same-flat neighbours (cumulated +8 / +16 height); all steps start together and their staggered travel produces the bottom-up ripple — the higher step owns each shared riser. **Closing doors** (close-stay and close-wait-reopen), **perpetual platforms** (with their stop lines, resumable stasis), **moving ceilings** (raise-to-highest with its companion floor lower, lower-to-floor and crush variants), **crushers** (slow/fast/silent, endless loop, stop lines) and the **donut** (tagged pillar lowers while the untagged ring around it rises to the outer level, taking its flat and losing its damage special on arrival, vanilla `EV_DoDonut` — E1M2's slime turns into safe floor; the ring answers to the donut special alone, never to another special sharing the tag) are all in as well. A door carries **one cycle per special aimed at it**, and the crossed line picks the one that runs: two open cycles on the same tag (E1M4's doors mix open-wait-close and open-stay lines), but also a closing line aimed at a door a switch already opened — which then plays that line's own cycle instead of a plain shutting, so the 30 s reopen of the close-wait-reopen specials still happens (E1M6's trap is no dead end), a crusher line still grinds in its endless loop (Heretic E4M9) and a lowerAndCrush ceiling still stops 8 units above the floor. A cycle is only applied while the panel sits at its first keyframe (an opening cycle rests closed, a closing one parked open), so switching cycles can never teleport a panel. Things standing on a moving floor ride it (a pickup on the donut pillar comes down with it). Movers press back Doom-faithfully: **crushers and crush floors deal 10 hp every 4 tics** to the squeezed player and keep moving through him (slow crushers throttle to 1/8 speed while crushing, fast ones don't), a blocked **door goes back up**, a blocked **lift goes back down**, waits and retries, and the close-stay family just leans on you harmlessly ("DO NOT GO BACK UP!") until you step aside — you can always walk out from under a pressing mover, and you are never pushed out of the map (the head stays clamped under the ceiling while a crusher kills). **Gun-activated lines** (G1/GR specials 24/46/47) fire when a hitscan trace crosses them in 2D, vanilla `P_ShootSpecialLine` style — even a shot passing above a low shootable wall: the shoot-door opens and stays open (repeatable GR), the gun-raised floors and plats use their vanilla targets (one-shot G1); a door mixing a shootable face and a manual face (E1M2's vent door) refuses USE from its gun side, and a switch graphic sitting on a mover's riser (flush while parked, MAP19) still works, through the same delegation to the mover's faces. **Monster-crossed lines** complete the set (vanilla `P_CrossSpecialLine`): a walking monster fires the walk doors and plats (4/10/88 — a once-only line is consumed for everyone, monsters included) and takes the teleporters — the shared 39/97 and the monster-only 125/126 (no player pad is built for those); a monster teleports to the landing facing its angle with all momentum cleared, fails silently if the spot is held (telefrag only on MAP30, vanilla), while a **player** arrival always telefrags whatever stands there. **Boss deaths** trigger too (vanilla `A_BossDeath`, table per game profile): when the last live boss of its kind dies on its map, the map action fires — tag-666/667 movers that often no linedef declares, so the converter builds them from a virtual line (E1M8's barons lower the block hiding the final pit, MAP07's mancubi lower the outer ring and its arachnotrons raise the tag-667 floor by its shortest lower texture, Heretic's E?M8 bosses open their tag-666 passage) — or the level simply exits (E2M8/E3M8, whose only exit is the boss's death).
...
- **Gamepad support**: press any button on a connected gamepad to use it (left stick to move, right stick to look — both analog). The pause button (`ESC` on the keyboard, button 9 on the gamepad) opens the pause menu over the frozen game; shoulder buttons 4/5 cycle the previous/next weapon.
...
| ESC | Button 9 | Pause menu (resume / load / save / options / leave the level) over the frozen game (not remappable — in game the browser consumes `ESC` to release the mouse, and that release is the pause signal) |
...
│   │   ├── save/
│   │   │   ├── doomSaveStore.js       Save slot persistence (saveMeta/saveData stores, 5 slots per WAD)
│   │   │   └── doomGameSnapshot.js    Capture/restore of a running level as JSON-safe data (deterministic rebuild + patch by instance code)
...
│   │   │   ├── doomBossDeath.js       A_BossDeath rules of the level: last boss of a def dies → tagged movers start or the level exits
│   │   │   ├── doomMonsterSystem.js   35 Hz runtime driver: state machine + actions (wake-up, chase, unblock, explode, boss death), senses, nightmare respawn, crossed-line firing, velocity/env integration, rotation views, render-glide blending, ray↔body cylinder tests
...
│   │   │   ├── menuModal.js     Confirm / info / loading / message modals
...
│   │   │   ├── menuSaveSlotsModal.js  Save slots modal (load/save modes, 5 slots, delete/overwrite confirmations)
│   │   │   ├── menuPauseModal.js      Pause modal over the frozen game (resume / load / save / options / leave the level)
...
│   │   │   ├── wadMenuScreen.js       Menu of one WAD (new game / load game / options / about / quit)
...
│   │           ├── wadLineCrossing.js     Walk-line crossing gate of a zone (P_CrossSpecialLine), player-box tolerance
