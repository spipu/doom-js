...
- **On-the-fly conversion** (`js/doom/wad/convert/`): level lumps, palette, picture/flat/texture decoding, animated sequences, sector floors and ceilings built from the WAD's own BSP (correct even on unclosed sectors, with triangulated polygons as fallback), movers with keyframes — all instantiated directly into in-memory engine objects.
