...
- **Weapons**: the nine Doom weapons fire — data-driven definitions plus the vanilla psprite state machine (`p_pspr.c` port, 35 Hz) with view-sprite bob, eased raise/lower switching and refire. **Hitscan** weapons (fist, chainsaw, pistol, shotguns, chaingun) cast one free-aim ray per pellet with vanilla spread and leave translucent puffs (bullets pass through transparent grates); **projectiles** (rocket, plasma, BFG) fly as billboards — energy shots additive-blended — and explode on impact, with the rocket's self-splash on the player. **Persistent impact decals** (UZDoom graphics: bullet chips, scorches, plasma burns, the fading BFG lightning) stay glued to the walls and ride moving doors/lifts — even when shot mid-travel — with a FIFO cap. The weapon view sprite is shaded by the player's sector brightness and pulses with its light effect.
...
│   │   │   ├── doomWeaponDef.js       The nine weapons: states + fire parameters (info.c / d_items.c)
...
│   │   │   ├── doomHitscan.js         Bullet rays: spread, puffs, decals, gun-trigger traces
│   │   │   ├── doomProjectile.js      Rocket / plasma / BFG shots, explosions, rocket self-splash
│   │   │   ├── doomEffects.js         Transient sprite effects (puffs, explosions) from batch-built templates
...
│   │   │   ├── doomDecalTextures.js   UZDoom decal PNG loading (level-independent)
│   │   │   └── doomDecals.js          Persistent wall decals: variants, FIFO cap, BFG fade, mover riding
│   │   └── wad/
...
│   │           ├── wadConstants.js        Doom specials tables, flags, pressure profiles, player defaults
│   │           ├── wadMapInfo.js          Level progression: vanilla G_DoCompleted defaults + UMAPINFO overlay
