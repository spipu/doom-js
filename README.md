...
- **Monsters** (display phase): every monster of the loaded game spawns from the THINGS lump — the full Doom/Doom 2 bestiary (zombies to the Cyberdemon, Spider Mastermind, Commander Keen and the MAP30 Boss Brain) and the Heretic one (gargoyles to the Maulotaur and D'Sparil's serpent), 35 definitions transcribed from the UZDoom zscript actors and owned by the game profiles. Each monster is a billboard with the **eight vanilla rotation views** (mirrored lumps like `TROOA2A8` are flipped at decode time, anchors mirrored with them), faces its THINGS angle, plays its idle animation at 35 Hz and blocks the player with its Doom hitbox; the Spectre and the Heretic ghosts render translucent. Monsters are inert for now — hunting, damage and death come with the next phases of the chantier.
...
│   │   ├── monster/            Monster system (display phase: static spawns)
...
│   │   │   └── doomMonsterSystem.js   35 Hz runtime driver: idle animation + camera-octant rotation view swaps
│   │   ├── hud/
