...
- **On-the-fly conversion** (`js/doom/wad/convert/`): geometry, textures, animations and movers are instantiated directly into in-memory engine objects — no generated file, no server. Two-sided middle textures show only from the side that carries them, and the holes of the original maps (missing upper or lower textures) are filled with the neighbouring flat, as the hardware ports do. The known defects of the original Doom, Doom II and Heretic maps (missing textures, misplaced sector heights, dead-end doors) are patched at load time from the hardware ports' compatibility catalogue, each map recognised by the fingerprint of its own lumps.
