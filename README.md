...
- **Difficulty selection**: after picking a WAD, a difficulty screen offers the five canonical Doom skills (*I'm too young to die* … *Nightmare!*) plus a skill 0, **Labyrinth but no monster** — the skill-1 world with every monster removed. The chosen skill drives the single-player thing filtering below and is kept across the level chain; the per-skill gameplay rules (spawn filter bit, ammo factor — Doom ×2, Heretic ×1.5 on the easiest/hardest skills — and the half player damage on the easy skills, applied before armor like vanilla) are game-profile data transcribed from the UZDoom MAPINFO skill blocks.
...
- **Help modal & persistent settings**: a `?` button on the WAD screen (also reached by pressing Up from the first list item) opens a stacked help modal — breadcrumb title, same mouse/keyboard/gamepad navigation as the screens — with an **About** page, a **Display** page (the `display.*` options — the aiming crosshair and the distance light diminishing toggles) and a **Controls** page that adapts to the active input device (named gamepad / virtual gamepad / keyboard+mouse). The settings lists are auto-built from a definitions table and persisted in IndexedDB: per-device vertical-look inversion, and **full keyboard remapping** (one physical key per action — activate a binding, press the new key; a key already bound elsewhere is unbound from its old action, which becomes inert until remapped). A reset entry wipes every saved setting after confirmation.
...
│   │   │   ├── difficultyScreen.js    Skill (0-5) selection between WAD and levels
...
│       │   ├── instance.js      Animated 3D object (keyframes, triggers, start/stop/reverse, damage, collisionShape none/faces/box, opaque triggerCondition gate, pressure behaviours stall/reverse/crush, programmatic zone firing, draw-only render offset)
