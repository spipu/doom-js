...
Status: fully designed; the generic network layer (`js/webapp/net/`, `js/webapp/qr/`) is written and runs on real devices through its test bench `_examples/pairing-test.html` and the screen-sharing demo `_examples/pairing-game.html` (step 0, device matrix partly run); steps 1 to 5 are done (the Multiplayer options section, nickname and game settings; the WAD identity; the per-turn commands; the simulation / presentation split with the players, their roster, the item rules and the single-player mode rules; several players in the engine and the game systems, spawning on the map's player starts); step 6 is in progress (see its plan in the step plan below); the later steps are not started. This section is the reference for the technology choices, the implementation and the step plan.
...
* **Multiplayer screen** (WAD main menu, above Options): Cooperative, Deathmatch, Join a game, and a shortcut to the Multiplayer options section. Delivered in two steps (user decision, 2026-09-26): step 6 brings the screen with Join a game and the options shortcut only, with the same greying and nickname rules as every multiplayer entry; step 8 adds Cooperative and Deathmatch.
...
* **Level build**:
  * player starts 1–4 and deathmatch starts collected through profile editor numbers, whatever the thing filter (vanilla `P_SpawnMapThing` handles the starts before the `MTF_NOT_SINGLE` test): a cooperative game opened on a level built for single player still spawns each player at the start of its slot;
  * a player spawns on a free start, as vanilla `G_CheckSpot` / `G_DoReborn` do: the start of its slot when no body stands on it, else another free player start, else its own start anyway — players block one another by pushing the moving body out of the other's box (resolved against the walls, so nobody is ever shoved through one), which matches vanilla at contact but lets a deep overlap be walked out of, so an overlap is never created on purpose;
  * the Heretic mace spot is chosen deterministically (seeded from an FNV-1a hash of the level's THINGS lump — synchronous and always available, unlike the SHA-256 map checksum which needs `crypto.subtle`, missing outside a secure context) instead of `Math.random`, so every device builds the same level; in solo, a given level always puts the mace at the same spot.
...
4. **Simulation / presentation split** (done): `DoomSimulation`, `DoomPresentation`, `DoomPlayer`, `DoomPlayerRoster`, `DoomSinglePlayerRules`.
5. **Several players in the engine and systems** (done): multi-user `World`, `Instance` trigger split with activator, per-user collision state, monsters / damage / traces / interactions / sounds over the roster, deterministic mace spot. Verified with a local second player driven by a scripted command. Groundwork found at the review of step 4, to do first: `DoomSimulation` mixes the level wiring and the player wiring (`enterLevel` sets the world and equips one player, `startLevel` wires the level systems and builds one weapon controller) — split it into a level `startLevel()` and a per-player `addPlayer(player, state)` before a second player exists (done); the `setWorld(collision, user)` of the monsters, the damage and the projectiles, and the `setUser` of `DoomMonsterTrace` / `DoomMonsterView`, carry one user and become roster-based; `DoomSimulation._worldBodyCommand` (the one body the engine world moves) goes away with the multi-user `World` (done: multi-user `World`, `Instance` trigger split with activator, per-user collision state; interactions over every player — pickups, teleports with per-player cooldown and player telefrag, walk-line crossings per actor, secrets, sector damage and push; monsters, damage, traces and sounds over every player — P_LookForPlayers with a lastlook starting at 0, the noise from the shooter, per-player save codes, friendly fire as a rule, player sounds positioned for the others; a dead player neither blocks nor picks up). Then (done): (a) level build for several players — a profile `playerStartTypes()` (1–4 for every current game), the builder collecting every player start whatever the thing filter and handing the slot → pose table to the simulation (`_computeSpawn` reads start 1 from it, `FALLBACK_SPAWN` kept), an engine `WorldLoader.createUser(position, yaw)` building an extra body from the loaded user definition, `DoomSimulation.addPlayer` giving any player beyond the main a new body on a free start (slot start if no living body overlaps, else another free start, else its own, else start 1 — slot = player id until the lobby of step 6), `DoomGame` carrying every roster player across level changes (`packForNextLevel` / `addPlayer` for each, main first), and the deterministic mace spot; (b) verification — solo benches identical to HEAD except the mace spot on Heretic levels that have one, and a scripted second player on Doom2 MAP01 (spawns at start 2, walks, picks up, crosses lines, is seen and hurt by monsters, blocks player 1, falls back to another free start when its own is taken, keeps its equipment on the next level), then README, docs and review.
6. **Mode 1, screen sharing**: pairing and lobby modals, pause menu entries, invite / answer codes, version and WAD checks, state and command binary codecs with fail-fast decoding, per-turn states and events (joining included, with the syncing / awaited switch), waiting message, sub presentation, level change follow-up, pause, save and load, stop / leave / lost link, ping in the fps readout. Groundwork found at the review of step 4, to do first, solo unchanged: (a) `WadWorldBuilder` receives the simulation and fills it through eleven setters, and the interactions it builds hold it — a sub builds the same level with no simulation, so the builder returns a level-data object (automap, sector light and damage, mover and ambient sounds, gun triggers, surfaces, terrain, totals) that the simulation adopts and the presentation reads (done: `DoomBuiltLevel`, returned by `build()` and adopted by the simulation; only the pickup, secret and teleport interactions still take the simulation, and exist on the main alone); (b) the HUD (`bindSimulation`: stats, item rules, profile) and `DoomPresentation.showLevel(simulation)` (world, automap, sounds) bind the simulation itself — on a sub those values come from the replicated state, so they bind a narrower level view; (c) the one-shot sounds are played straight from the simulation side (`doomSound.play*` in the hitscan, the projectiles, the monsters, the item rules, the switch and secret interactions, the terrain, the effects, the weapon — fourteen files): they become the turn's events, emitted through one collector the simulation fills and the presentation plays locally or encodes; (d) `Math.random` stays in the sector light and damage thinkers (main-only, replicated by state) and in the mace spot (deterministic seed, step 5) (checked at the start of step 6: nothing to change, the sound-side `Math.random` calls being presentation only).

   Plan of step 6, agreed on 2026-09-26 — one commit per lot, each one reviewed (`/doom-review`) and keeping solo bit-identical (full-loop bench: trace and end-of-trace save hashed against HEAD on Doom2 MAP01, Heretic E1M1 and E1M3; converter bench over the 6 WADs whenever the converter moves):

   * **6.0a — built level** (done): see (a) above.
   * **6.0b — level view**: the HUD (`HudDoom` / `HudGameBar` / `HudDoomDebug` / `HudAutomap`, today `bindSimulation`) and `DoomPresentation.showLevel` bind one narrow interface — game profile, item rules (weapon catalogue, map power-up), level stats (secrets, kills, items found and totals), world, automap, mover and ambient sounds, the viewer of the bodies — implemented by `DoomSimulation` on the main, and by the replicated state on a sub (lot 6.3).
   * **6.0c — sound events**: one collector, `DoomSoundEvents`, injected by the simulation into the eleven files that play one-shot sounds today (`doomItemRules`, `doomUser`, `doomMonsterSystem`, `doomSecretInteraction`, `doomSwitchInteraction`, `doomTerrain`, `doomEffects`, `doomGunTriggers`, `doomHitscan`, `doomPlayerWeapon`, `doomProjectile` — 25 calls, none of which uses the returned handle): sound at a position, sound of a body (today a live `getWorldCenter()` reference, which is how a monster's voice follows it), sound of a player, sound for one player's ears. On the main it plays at once, locally, exactly when it plays today — solo audio unchanged; in a session it also records the event for the turn message (a body is then named by its network id, which arrives with lot 6.2). The mover loops and the ambient sequences stay presentation-side (the sub derives mover loops from pose changes).
   * **6.1 — network in the game and menus**: `js/webapp/net`, `js/webapp/qr` and zxing declared in the doom `libBootstrap.json`; `DoomNetProtocol`, `DoomNetInvite`; `MenuPairingModal` (the view the pairing flows drive, camera and QR code); `MenuLobbyModal` + `DoomNetLobby`; the reduced Multiplayer screen (Join a game + options shortcut); pause entries "Share screen" (first entry), "Stop sharing", "Lobby" for the main, "Leave" for a sub; empty-nickname modal; greying without camera or WebGL; WebGL forced during a session; explicit refusal on a version or WAD mismatch; every label in en / fr / it / es.
   * **6.2 — replication**: `DoomNetEntityIds`, `DoomNetStateCapture`, `DoomNetStateCodec`, `DoomNetCommandCodec`, `DoomNetEvents`; encode → decode round-trip bench measuring the largest error of every float field.
   * **6.3 — synchronous cycle and sub**: `DoomNetHost` (syncing / awaited, switch on the next state sent after `levelReady`, 5 s command timeout, waiting message), `DoomNetClient`; the sub builds the level with no simulation (`WadWorldBuilder` with `simulation: null`), `DoomReplicaApplier`, the presentation bound to the replicated level view with the camera on the main's player; liveness timeout suspended during the build. To handle there: the monster drops are only registered by the builder when a simulation exists (`_registerMonsterDrops`), so a sub needs their billboards to show replicated drops.
   * **6.4 — full flow**: `levelLoad` / `levelReady` follow-up, `pause` / `intermission` / `finale` screens without action on the subs, death of the main ("The main is dead", restart and load followed as a level change, new game or quit ends the sharing), save and load on the main only, stop / leave / lost link back to the WAD menu with an information modal, ping in the fps readout.
   * **6.5 — verification**: every flow driven through the real menus in two tabs over the loopback link (Doom2 and Heretic: pairing, joining mid-level, level change, pause, tally, death, load, stop, lost link), per-stage profiling (capture, encode, size, decode, apply), device matrix on doom.spipu.net run by the user, README, docs, review.
   * **Loopback switch for tests** (user decision, 2026-09-26): a development URL parameter `?netLoopback=1`, read once at start-up and invisible in the menus, makes the pairing use `NetLoopbackLink` + `NetLoopbackCodeChannel` instead of WebRTC and the camera; the sessions take an injected link factory, so the parameter is read at one single launch point and nothing else knows about the loopback. **It is removed before every commit**: loopback tests first, then removal and a new bump, then the non-regression without it (syntax, solo benches, real menus in WebRTC mode up to the QR code), the removal being stated in the report.
