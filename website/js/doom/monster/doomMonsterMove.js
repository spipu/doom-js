...
class DoomMonsterMove {
    ...
    move(m) {
        ...
        let   attempt = this._tryMove(m, destX, destZ);
        if (!attempt.ok && isFloat && !attempt.floatok) {
            ...
            const destFloorY = this._collision.getFloor(destX, destZ, r, Infinity);
            if ((destFloorY !== -Infinity) && (destFloorY > pos[1])) {
                const destCeilY = this._collision.getCeiling(destX, destZ, r, destFloorY + 0.01);
                if ((destCeilY - destFloorY >= m.def.getHeight() * S)
                    && !this._blockedAboveLedge(m, pos, destX, destZ, destFloorY, r)) {
                    attempt = {ok: false, floor: destFloorY, floatok: true};
                }
            }
        }
        if (!attempt.ok) {
            if (isFloat && attempt.floatok) {
                ...
                m.inst.translate(0, ((pos[1] < attempt.floor) ? WadConstants.ACTOR_FLOAT_SPEED : -WadConstants.ACTOR_FLOAT_SPEED), 0);
    ...
    // P_NewChaseDir: a body hanging over a drop deeper than one step first
    // walks AWAY from it — the strict dropoff test would otherwise pin it there.
    newChaseDir(m) {
        if (m.target === null) {
            m.movedir = DoomMonsterMove.DI_NODIR;
            return;
        ...
        if (escape !== null) {
            // MF5_AVOIDINGDROPOFF is an actor flag, so it rides the record.
    ...
    // A direction is ATTEMPTED ONCE only: a retry would draw extra numbers from
    // the shared random table.
     ...
     * the normals of the contacted dropoff linedefs; with no linedef list at
     * runtime, the four box corners are probed and the escape points away from
     * those hanging over the void.
     *
    ...
    // factor (ice ≥ ORIG_FRICTION, mud below).
...
DoomMonsterMove.XSPEED       = [1, DoomMonsterMove.SQRTHALF, 0, -DoomMonsterMove.SQRTHALF, -1, -DoomMonsterMove.SQRTHALF, 0, DoomMonsterMove.SQRTHALF];
DoomMonsterMove.YSPEED       = [0, DoomMonsterMove.SQRTHALF, 1, DoomMonsterMove.SQRTHALF, 0, -DoomMonsterMove.SQRTHALF, -1, -DoomMonsterMove.SQRTHALF];
