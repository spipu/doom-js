...
class WadGeometry {
    ...
    // Distance along the ray (px, py) + t·(dx, dy), 0 ≤ t ≤ maxDist, where it
    // meets the segment (ax, ay)-(bx, by); null when it misses.
    static raySegmentHit(px, py, dx, dy, maxDist, ax, ay, bx, by) {
        const sx = bx - ax;
        const sy = by - ay;
